...
(function ($) {
    ...
    $(function () {
        ...
        Authors().init();
        var Card = function () {
            var bookTemplate = function (bookEntity) {
                return '<div class="Card">' + '   <div class="Card-picture">' + '<a href="' + bookEntity.slug + '">' + '       <img src="' + bookEntity.image + '">\n' + '</a>' +
                    (bookEntity.discount ? '<div class="Card-sale">' + bookEntity.discount + '% скидка</div>' : '') +
                    (bookEntity.isBestseller ? '<div class="Card-ribbon"><span class="Card-ribbonText">Бестселлер</span></div>' : '') +
                    (bookEntity.status === 'PAID' ? ' <a class="Card-status" title="Куплена"><img src="/assets/img/icons/check.svg" alt="Куплена"></a>\n' : '') +
                    (bookEntity.status === 'CART' ? ' <a class="Card-status" title="В корзине"><img src="/assets/img/icons/shopping-basket.svg" alt="В корзине"></a>\n' : '') +
                    (bookEntity.status === 'KEPT' ? ' <a class="Card-status" title="Отложена"><img src="/assets/img/icons/heart.svg" alt="Отложена"></a>\n' : '') + '    </div>' + '              <div class="Card-content">\n' + '                <strong class="Card-title"><a href="#">' + bookEntity.title + '</a>\n' + '                </strong>\n' + '                <div class="Card-description">' + bookEntity.authors + '                </div>\n' + '                <div class="Card-cost">' +
                    (bookEntity.discountPrice ? '<span class="Card-priceOld">₽' + bookEntity.discountPrice + '</span>\n' : '') + '                       <span class="Card-price">₽' + bookEntity.price + '</span>\n' + '                </div>\n' + '              </div>\n' + '            </div>';
            }
            return {
                init: function () {
                        ...
                        switch (data) {
                            ...
                            case'popular':
                                getData('/api/books/popular', {
                                    offset: function (){
                                        var oldOffset = $this.data('refreshoffset');
                                        var limit = $this.data('refreshlimit');
                                        if (oldOffset < 1) {
                                            $this.data('refreshoffset', +1);
                                        } else {
                                            var x = $this.data('refreshoffset');
                                            var y = $this.data('refreshlimit');
                                            $this.data('refreshoffset', x - (y -1) );
                                        }
                                        return $this.data('refreshoffset');
                                    },
                                    limit: $this.data('refreshlimit')
                                    ...
                                    if ($(slick.$slider).data('load')) {
                                        ...
                                        switch ($(slick.$slider).data('load')) {
                                            case'recommended':
                                                ...
                                                break;
                                            case'recent':
                                                sendData('/main/recent')
                                                break;
                                            case'popular':
                                                sendData('api/books/popular')
                                                break;
                                            case'authors':
                                                sendData('/authors'+ $this.data('refreshid'))
                                                break;
