package com.example.springbootnewsportal.configuration;

import io.swagger.v3.oas.models.OpenAPI;
import io.swagger.v3.oas.models.info.Contact;
import io.swagger.v3.oas.models.info.Info;
import io.swagger.v3.oas.models.info.License;
import io.swagger.v3.oas.models.servers.Server;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;

import java.util.List;

@Configuration
public class OpenAPIConfiguration {
    @Bean
    public OpenAPI openApiDescription() {
        Server localhostServer = new Server();
        localhostServer.setUrl("http://localhost:8080");
        localhostServer.setDescription("Local env");

        Server productionServer = new Server();
        localhostServer.setUrl("http://some.prod.url");
        localhostServer.setDescription("Production env");

        Contact contact = new Contact();
        contact.setName("Ivan Ivanov");
        contact.setEmail("some_email@org");
        contact.setUrl("http://my_url.url");

        License mitLicence = new License().name("GNU AGPLv3")
                .url("https://choosealicense.com/licenses/agpl-3.0");

        Info info = new Info()
                .title("Users posts API")
                .version("1.0")
                .contact(contact)
                .description("API for user posts")
                .termsOfService("http://some.terms.url")
                .license(mitLicence);
        return new OpenAPI().info(info).servers(List.of(productionServer, localhostServer));

    }
}
